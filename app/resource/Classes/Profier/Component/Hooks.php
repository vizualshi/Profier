<?php
/**
 * Hookable Function implemented for interactive result of method / functions
 * this class using implementation like a WordPress do.
 *
 * @author nawa <nawa@yahoo.com>
 * @version 1.0
 * @license  Follow WordPress GPLv2 or later
 */
namespace Pentagonal\Profier\Component;

use Pentagonal\Profier\Abstracts\HookAbstract;

/**
 * This Class Running like as a WordPress Hook
 * @license  Follow WordPress GPLv2 or later License
 * @see  {@link: https://wordpress.org/license} for more related WordPress License
 */
class Hooks extends HookAbstract
{
    /**
    ...
    public function add($hookName, $callable, $priority = 10, $accepted_args = 1, $append = true)
    {
        ...
        $hook_list[$hookName][$priority][$id] = [
            'function' => $callable,
    ...
    public function exists($hookName, $function_to_check = false)
    {
        ...
        if (false === $function_to_check || false === $has ||
            ! $id = $this->uniqueId($hookName, $function_to_check, false)
        ) {
            return false;
        }
